...
> **Kimen note (local adaptation):** this workspace uses pnpm — invoke Nx as
> `pnpm exec nx …`. Kimen components (`ki-*`) are scaffolded via Nx generators,
> never by hand (AGENTS.md conventions); generated artifacts are regenerated,
> not patched (constitution Art. I). The `nx_docs` MCP tool mentioned in the
> description is not installed in Kimen — skip that step.
