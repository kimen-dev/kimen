...
> **Kimen note (local adaptation, constitution Art. XI):** the "Getting Reference
> Code" step below runs `git clone` of the MCP ext-apps repo, and other steps run
> `npm install`. Run those network+install steps ONLY inside a credential-free,
> sandboxed, disposable environment — never on a machine with publishing tokens
> or long-lived secrets in the environment. In Kimen this maps to MCP Apps
> adapters (Art. VIII disposable adapters); use pnpm rather than npm where you
> integrate into a Kimen package.
...
> **Kimen note:** Kimen uses pnpm and pins dependencies (constitution Art. X
> supply-chain gate + Renovate). When integrating into a Kimen package, use
> `pnpm add` and let the lockfile pin; treat any newly added runtime dependency
> as subject to the audit-and-pin gate.
