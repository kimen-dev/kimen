...
>
> **Kimen note:** vendored from `anivar/zod-skill` (MIT). This compiled guide is
> the consolidated form of the upstream `rules/` and `references/` trees; it is
> the source of truth for the vendored `zod` skill. See `SKILL.md` for the Kimen
> catalog/boundary framing.
...
> **Kimen note:** for Kimen this "boundary" is where adapter payloads (A2UI, MCP
> Apps, AG-UI, json-render) and external catalog input enter — parse there, hand
> typed catalog entries to the neutral catalog/domain (Art. VIII).
...
MIT — © Anivar Aravind. See LICENSE-NOTICE.md for vendoring provenance.
