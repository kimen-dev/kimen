/* eslint-disable */
/* tslint:disable */
/* auto-generated vue proxies */
import { defineContainer, type StencilVueComponent } from '@stencil/vue-output-target/runtime';

import type { JSX } from '@kimen/elements/components';

import { defineCustomElement as defineKiDialog } from '@kimen/elements/components/ki-dialog.js';



export const KiDialog: StencilVueComponent<JSX.KiDialog> = /*@__PURE__*/ defineContainer<JSX.KiDialog>('ki-dialog', defineKiDialog, [
  'open',
  'heading',
  'closeOnBackdrop',
  'ki-close'
], [
  'ki-close'
]);

