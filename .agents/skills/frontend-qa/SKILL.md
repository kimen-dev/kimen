...
scripts/gates/gates-suite.sh as the fast definition of done, daily mutation
quality feedback, S-ID traceability where a contract exists, Kimen catalog
naming.
...
Core stance: a test suite exists to let you change code without fear. Every rule below serves that goal; any practice that doesn't (coverage worship, snapshot dumps, gherkinized unit tests) is cut. Public behavior contracts come first when Article II requires them, and tests derive from those scenarios. Bug fixes need a failing regression test, not a new spec. In Kimen, the fast quality suite exiting 0 is the ordinary completion signal (Art. III/X).
...
- **[Always]** Mutation remains the strongest test-quality signal for core logic: **at least 70% on core code changed in the preceding 24 hours**. It runs once daily, never repeatedly in PR or local completion loops. If mutating the code doesn't fail a test, focus the next test work there. Don't run it on glue/presentation code — wrong tool.
...
- **[Always]** S-ID traceability (Art. II): when a behavior contract exists, every scenario ID (S1..Sn) appears in at least one test at the right level. `scripts/gates/check-traceability.sh` checks this, and the test must assert the observable outcome — not merely name-drop the ID.
...
The ordinary PR pipeline exposes one required `quality` result. `scripts/gates/gates-suite.sh` runs the high-signal core once: constitution sync, format, lint (code + styles), strict typecheck, module boundaries, build, public API/token integrity, unit/component tests, Chromium, axe and budgets. S-ID traceability applies only where an Article II contract exists. Packaging and the three-browser matrix belong to release; mutation runs daily; containment and dependency review are path-scoped; broad security scans run on `main` and weekly. Add a gate only when its defect-detection value exceeds its runtime and maintenance cost.
...
|---|---|---|
| Coverage targets (100%, 90%…) | Breeds assertion-free tests | Daily mutation signal on core logic |
...
| "The tests pass, so I'm done" | Run the consolidated fast quality suite: it also checks types, API/token integrity, Chromium, axe and budgets (Art. III/X). |
| "The reviewer approved it, so it's done" | Review is optional advice. Quality must be green and merge remains the founder's decision. |
...
- Declaring done from your own assessment or a reviewer's without the fast quality suite exiting 0

## Definition of done (testing)

For an ordinary change, done means `scripts/gates/gates-suite.sh` exits 0 and the founder accepts the change. Verify:

1. When an Article II `feature.feature` exists, every S-ID has a test at the right level asserting the observable outcome.
...
4. New/changed core logic is inside the daily mutation scope; do not rerun mutation to finish the PR.
