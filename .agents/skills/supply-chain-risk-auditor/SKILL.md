...
> **Kimen note (local adaptation):** in Kimen this pairs with the constitution's
> supply-chain gate (Art. X): a new runtime dependency passes audit + pin before
> it enters. Run any `gh` queries in a credential-free sandbox (Art. XI); never
> run this with a publishing token in the environment.
