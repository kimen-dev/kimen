...
> **Kimen note (local adaptation):** Kimen is a pnpm workspace
> (`pnpm-workspace.yaml`, packages under `packages/*` with the `@kimen/`
> scope) — the **pnpm** section below is the one that applies here.
