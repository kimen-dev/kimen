...
Constitutional skill: the executable elaboration of Art. II (Proportionate
Behavior Contracts) of the constitution digest
(`.specify/memory/constitution.md`). On conflict, the constitution wins.

Public component behavior/API, a new interaction pattern, or a guardrail and
security-boundary change enters the system as a concise `.feature` before
implementation. The feature file is an agent-readable contract: badly written
Gherkin poisons every agent that consumes it.

## When to use / not use

Use when Article II requires a contract; when writing the Gherkin section of a
spec, creating or editing `specs/<feature>/feature.feature`, confirming
scenarios with the founder, or mapping scenarios to tests.

Do NOT gherkinize: unit/implementation tests ("Given the validator receives an
empty string" is a Vitest test, write it there); bug fixes (write a failing
regression test), internal refactors, dependencies, tooling, docs and low-risk
maintenance. Do not create a feature file merely to satisfy process.
...
guardrail or security boundary require explicit founder confirmation.
...
- [ ] Guardrail/security-boundary scenarios flagged for explicit confirmation
- [ ] Open questions batched for the founder
