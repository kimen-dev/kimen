...
- The "conversation" is the founder confirming intent for the public behavior
  or boundary. Scenarios must be readable in minutes, hence BRIEF.
- The `.feature` is an agent contract: implementing agents derive tests from
  it and the traceability gate binds every S-ID to a test. Imperative or vague
  Gherkin poisons both consumers.
...
- the constitution digest (`.specify/memory/constitution.md`) Art. II (Proportionate Behavior Contracts), Art. III, V, VI, VIII
