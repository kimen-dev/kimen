...
> **Kimen note:** The DTCG Resolver spec below is the standard theming model.
> Kimen's build tool is **Style Dictionary 5**, which does not consume resolver
> files directly — model themes as multiple Style Dictionary source sets or
> per-theme configs (see `tools.md`). This reference is kept for spec fidelity and
> for interoperating with resolver-aware tools.
