...
> **Kimen note:** examples below use spec-preferred structured DTCG values and,
> in places, Terrazzo/resolver build commands from the upstream skill. In Kimen,
> build with Style Dictionary 5 (see `reference/tools.md`); the token *shapes*
> here are still valid DTCG and translate directly. Nest under a top-level `ki`
> group when adding to the Kimen source.
...
- Stick to a single unit in source; transform per platform at build time
...
**Solution using Resolvers (DTCG spec):**
...
**Kimen build equivalent:** since Style Dictionary doesn't read resolver files,
build each brand as a separate Style Dictionary config whose `source` includes
the base plus the brand override file, emitting `partner-a.css` etc. See
`reference/tools.md` "Multiple output files / themes".
...
### Reduced motion support (Kimen: WCAG 2.2 AA)
```json
{
  "modifiers": {
    "motion": {
