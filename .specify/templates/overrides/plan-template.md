...
Use this template only when the change is complex enough to benefit from a plan.
...
- **Art. II — Proportionate behavior contracts (NON-NEGOTIABLE)**: [the `feature.feature` this plan implements and scenario families covered; confirm this change is public behavior/API, a new interaction or a security boundary]
- **Art. III — Test-first, deterministic gates (NON-NEGOTIABLE)**: [RED tests planned before implementation; fast quality coverage; real-browser and deterministic tests through public APIs; daily mutation scope when core logic changes]
...
- **Art. VII — Simplicity & anti-abstraction**: [simplest design satisfying the contract; no speculative props or premature generality; duplication before the wrong abstraction]
- **Art. VIII — Neutral catalog, disposable adapters**: [protocol/adapter surface touched; no protocol type leaks into @kimen/elements; guardrail boundary impact (→ standalone scenario) or "none"]
...
- **Art. X — Deterministic 20/80 automation (NON-NEGOTIABLE)**: [which existing high-signal checks cover the surface; add a repository-wide gate only for repeated/high-impact defect classes with justified ongoing cost]
...
**Definition of done (Art. III)**: the consolidated fast quality suite exits 0
(`bash scripts/gates/gates-suite.sh`) and the founder accepts the change.
Mutation is daily; packaging and the full browser matrix are release checks.
...
| [e.g., speculative prop] | [specific problem] | [why the contract requires it] |
